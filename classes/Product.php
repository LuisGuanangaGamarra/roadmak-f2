...
class ProductCore extends ObjectModel
{
    ...
    public function add($autodate = true, $null_values = false ,$name=array())
    {
        ...
        if(!empty($name)){
            curl_setopt_array($curl, array(
                CURLOPT_PORT => "3988",
                CURLOPT_URL => "http://localhost:3988/api/registerConsultoria_prestashop",
                ...
                CURLOPT_POSTFIELDS => "name=".$this->id."&descripcion=".$this->name[1]."&precio=2",
                CURLOPT_HTTPHEADER => array(
                  "Content-Type: application/x-www-form-urlencoded",
                  "Postman-Token: 62272318-36c3-49e1-907a-374cc6a3e108",
    ...
    public function update($null_values = false)
    {
        ...
        $id_subconsultoria = Db::getInstance()->executeS('
        SELECT p.`grupo`
        FROM `subconsultorias` p
        WHERE p.`grupo` = '.$this->id);

        if (empty($id_subconsultoria)) 
    {
        ...
        if (!empty($this->name)
            ...
            && !empty($this->price))
        {   
        $id_name = Db::getInstance()->executeS('
            SELECT p.`name`
            FROM `'._DB_PREFIX_.'product_lang` p
            WHERE p.`id_lang` = 1 and p.`id_product` = '.$this->id); 

            $total = count($id_name);

            for ($i = 0; $i < $total; $i++) {
                
                /*inicio del curl de subconsultoria*/
                $curl = curl_init();

                curl_setopt_array($curl, array(
                CURLOPT_PORT => "3988",
                CURLOPT_URL => "http://localhost:3988/api/savesubconsultorias",
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_ENCODING => "",
                CURLOPT_MAXREDIRS => 10,
                CURLOPT_TIMEOUT => 30,
                CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
                CURLOPT_CUSTOMREQUEST => "POST",
                CURLOPT_POSTFIELDS    => "name=".$id_name[$i]['name']."&id_product=".$this->id."&precio=".$this->price."",
                CURLOPT_HTTPHEADER => array(
                    "Content-Type: application/x-www-form-urlencoded",
                    "Postman-Token: 4de0c013-70fb-404d-b9af-426c3d8b1e72",
                    "cache-control: no-cache"
                ),
                ));
                $response = curl_exec($curl);
                ...
                echo "cURL Error #:" . $err;
                    ...
                    /*fin del curl de subconsultoria*/   
